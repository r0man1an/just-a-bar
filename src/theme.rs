...
fn gnome_color_scheme() -> Option<ColorScheme> {
    ...
    Some(if value == "prefer-dark" { ColorScheme::Dark } else { ColorScheme::Light })
}
...
pub fn init() -> (ColorScheme, Option<gio::DBusProxy>) {
    let proxy = match gio::DBusProxy::for_bus_sync(
        ...
        Ok(proxy) => proxy,
        Err(err) => {
            eprintln!("jbar: no settings portal ({err}); defaulting to light theme");
            return (gnome_color_scheme().unwrap_or(ColorScheme::Light), None);
        }
    };

    let initial = if let Some(scheme) = gnome_color_scheme() {
        scheme
    } else {
        let read_result = proxy.call_sync(
            "Read",
        ...
        match &read_result {
            Ok(reply) => unwrap_scheme_variant(&reply.child_value(0)).unwrap_or(ColorScheme::Light),
            Err(err) => {
                eprintln!("jbar: settings portal Read failed ({err}); defaulting to light theme");
                ColorScheme::Light
            }
        }
    };
